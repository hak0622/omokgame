...
public class ServerWindow extends JFrame {
	...
	public void command(Client source, String data){
		...
		switch(PROMISE.valueOf(tokens.nextToken())){
			case LOGIN:
				...
				break;
			case REGISTER:
				send(source, register(tokens.nextToken(),tokens.nextToken(),tokens.nextToken(), tokens.nextToken(),tokens.nextToken(),tokens.nextToken(),tokens.nextToken(),tokens.nextToken(),tokens.nextToken(),tokens.nextToken(),tokens.nextToken(),tokens.nextToken(),tokens.nextToken()), cmdDetail);
				break;
			case ROOM:
			...
			case CHECK_DUPLICATE:
	            send(source, checkDuplicate(tokens.nextToken()), cmdDetail);
	            break;		
			case ADMIN_LOGIN:
			    String adminPassword = tokens.nextToken();
			    if (confirmAdminPassword(adminPassword)) {
			        // 관리자 비밀번호가 올바른 경우
			        cmdDetail = "관리자 로그인 성공";
			        System.out.println("Sending: " + PROMISE.ADMIN_LOGIN_SUC + ", " + cmdDetail);  // Add this line
			        send(source, PROMISE.ADMIN_LOGIN_SUC, cmdDetail);
			    } else {
			        // 관리자 비밀번호가 틀린 경우
			        cmdDetail = "관리자 비밀번호가 잘못되었습니다.";
			        System.out.println("Sending: " + PROMISE.ADMIN_LOGIN_FAIL + ", " + cmdDetail);  // Add this line
			        send(source, PROMISE.ADMIN_LOGIN_FAIL, cmdDetail);
			    }
			    break;
			default:
				break;
	...
	private boolean confirmAdminPassword(String adminPassword){
	    // DB에서 관리자 비밀번호를 확인하고 결과를 반환합니다.	  
	    // 예를 들어, DBConnection 객체를 사용하여 DB에서 관리자 비밀번호를 확인할 수 있습니다.	   
	    return adminPassword.equals("1111");  // "adminPassword"는 예시입니다.
	}
	
	private PROMISE checkDuplicate(String id){
	    cmdDetail = "";
	    if(conn == null || !conn.isConnect) return PROMISE.CHECK_DUPLICATE_FAIL;

	    if(conn.isUser(id)){
	        cmdDetail = "아이디가 중복되었습니다.";
	        return PROMISE.CHECK_DUPLICATE_FAIL;
	    }

	    cmdDetail = "아이디 사용이 가능합니다.";
	    return PROMISE.CHECK_DUPLICATE_SUC;
	}
	...
	private PROMISE register(String id, String pw,String name, String gender,String email, String zip,String address, String extraAddress,String profilePicPath,String birth, String ssn,String phone,String introduction){
		cmdDetail = "서버 오류!!";
		...
		if(conn.registerUser(id, pw,name,gender,email,zip,address,extraAddress,profilePicPath,birth,ssn,phone,introduction)){
			cmdDetail = "";
